...
var filename = 'Back.to.the.Future.1985.720p.BrRip.x264.YIFY.mp4';
...
submarine.get([origin_types_1.default.ORIGIN.SUBDIVX /* , TYPES.ORIGIN.OPEN_SUBTITLES */], filename, ['es', 'en'])
    .then(subs => {
    log.cInfo(logger_1.default.BLUE_BRIGHT, "Total length: ", subs.length);
    subs.map(sub => {
        log.info(`(${sub.origin}) ${sub.meta.title} -> ${sub.url}`);
    });
    submarine.download(subs[0], 'd:/downloads');
});
