        ...
        return new Promise((resolve, reject) => {
            ...
            fs_1.stat(path, function (err, stat) {
                if (err)
                    return reject(err);
                file_size = stat.size;
                fs_1.open(path, 'r', function (err, fd) {
                    if (err)
                        return reject(err);
                    var t_offsets = [0, file_size - chunk_size];
                    for (var i in t_offsets) {
                        b_read = fs_1.readSync(fd, buf, b_read, chunk_size, t_offsets[i]);
                    }
    ...
    static openSubtitlesHash(path) {
        ...
        return new Promise((resolve, reject) => {
            ...
            fs_1.stat(path, (err, stat) => {
                ...
                fs_1.open(path, 'r', (err, fd) => {
                    if (err)
                        return reject(err);
                    fs_1.read(fd, buf_start, 0, chunk_size * 2, 0, (er1, bytesRead, buf1) => {
                        fs_1.read(fd, buf_end, 0, chunk_size * 2, file_size - chunk_size, (er2, bytesRead, buf2) => {
                            fs_1.close(fd, er3 => {
                                if (er1 || er2)
                                    return reject(er1 || er2); //er3 is not breaking
